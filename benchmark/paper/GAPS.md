...
## 2026-07-03 增补 2：bottomleft / bottomright 视角处置（决策记录）

**背景**：人工审查发现 bottomleft/bottomright 两路相机距演员过近，部分片段上半身或下半身
完全出画。提出是否删除这两路。

**决议（原则先行，数字定案）**：

1. **不删除任何已采集数据**——问题拆为三层：官方评测是否使用（可不用）、
   发布集是否包含（包含但标注）、原始数据是否保留（永远保留）。
2. **程度用测量回答，不靠印象**：利用动捕 GT 投影到各相机，统计每路相机
   逐帧"关节在画面内比例"（帧内可见关节数 / 22）分布 → **G24**。
   与 G20 遮挡脚本共用同一套投影代码。
3. **预设落点（若数字确认较差）——分级而非删除**：
   - **Core 5 views**（topleft / topcenter / topright / bottomcenter / diagonal）：
     官方 X-View 协议仅用这五路；
   - **Auxiliary 2 views**（bottomleft / bottomright）：照常发布，附
     **逐帧关节可见性掩码**（由 GT 投影生成），标注为"近距离部分视野视角，
     不参与官方评测"。
4. **论文叙事**：从"两路质量差的相机"转写为"额外提供两路真实近距离
   部分视野视角及其可见性标注"——部分可见下的动作识别是被研究不足的
   附加研究条件；可见性掩码只有动捕真值数据集能提供，是精度族独有卖点。
   对比表口径改为 "7 calibrated views (5 core + 2 auxiliary)"。
5. **仅当** core 判定阈值都不满足（如全身可见帧中位数 < 50%）才考虑
   从发布集整体移除，且原始数据仍归档。

| # | 缺口 | 负责 | 来源 |
|---|------|------|------|
| G24 | 每相机逐帧关节可见性统计（决定 core/aux 分级 + 生成发布用可见性掩码） | Angus | 与 G20 同脚本，半天-一天 |

**联动改动（待 G24 出数后执行）**：main.tex 摘要/贡献/对比表的 "seven cameras" 口径、
X-View 协议定义、§3.5 加可见性统计、benchmark/ 增加 views.md v1 权威定义。
