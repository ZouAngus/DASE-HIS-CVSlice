...
```bash
python tools/qc_scan.py <演员或场景文件夹> [--mosh <mosh目录>]
```

每个场景生成 `qc_report.json`，分数 0-100（越高越差），信号 = 骨长偏离 + 速度尖峰 + 「身体在动但关节冻结」 + NaN 缺失。终端会打印每场景最差的 10 个片段。

2. Corrector 里自动生效：
- 动作列表名字前显示 `[分数]`；勾选「按 QC 分排序 (最差先)」= 修正顺序即排序顺序；
- `N` 键跳到本片段下一个可疑帧（循环），不用再逐帧扫着找问题。

没有报告时一切照旧。报告只影响顺序与导航，**从不改动骨架数据**。
